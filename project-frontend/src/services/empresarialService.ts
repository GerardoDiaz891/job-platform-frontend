...
export const EmpresarialService = {
  ...
  async downloadApplicantCV(idVacante: number): Promise<Blob> {
    try {
  ...
  async getMisVacantes(): Promise<VacanteDTO[]> {
    try {
      const { data } = await apiClient.get<VacanteDTO[]>('/api/Vacantes/mis-vacantes');
      return data;
    } catch (error) {
      const axiosError = error as AxiosError;
      console.error('Error al obtener vacantes empresariales:', axiosError.response?.data);
      throw axiosError;
    }
  },
};